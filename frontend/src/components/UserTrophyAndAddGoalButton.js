...
export default function UserTrophyAndAddGoalButton() {
	const { userCompletedGoals, userNotCompletedGoals } = useContext(UserContext);

	const hasCompletedGoals = Array.isArray(userCompletedGoals) && userCompletedGoals.length > 0;
	const canAddMoreGoals = Array.isArray(userNotCompletedGoals) && userNotCompletedGoals.length < 8;
	...
	if (!hasCompletedGoals && !canAddMoreGoals) {
		return null;
	}
	return (
		<>
