...
export default function UserGoalIndependentTasksButton() {
	...
	function openEditor() {
		const initialTasks =
			userGoalIndependentTasks.length > 0 ? userGoalIndependentTasks.map((task) => createDraftTask(task)) : [createDraftTask()];
	...
	function updateTask(localKey, field, value) {
		setDraftTasks((prev) => prev.map((task) => (task.localKey === localKey ? { ...task, [field]: value } : task)));
	}
